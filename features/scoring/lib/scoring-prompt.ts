...
function getCompanySizeFromRange(employeeRange: string): CompanySize {
  ...
  if (
    normalized.includes("1-50") ||
    normalized.includes("1-10") ||
    normalized.includes("11-50")
  ) {
    return "startup";
  }
  if (
    normalized.includes("51-200") ||
    normalized.includes("51-100") ||
    normalized.includes("101-200")
  ) {
    return "smb";
  }
  if (
    normalized.includes("201-500") ||
    normalized.includes("501-1000") ||
    normalized.includes("201-1000")
  ) {
    return "mid_market";
  }
  if (
...
export const BASE_PROMPT = `You are an expert B2B sales lead scoring system for Throxy, a company that helps B2B businesses with outbound sales.
...
- **Industry**:${lead.accountName} | ${lead.accountIndustry || "Not specified"}
`;
}
